...
Ten task-shaped guides follow. Each is a single file that runs offline without
...
| [`01_holdings_for_an_address.py`](01_holdings_for_an_address.py) | Priced portfolio for one address: exact `Decimal` totals, unpriced assets named instead of zeroed, the wire form, and the one line that points it at mainnet. | core |
...
| [`06_isolate_two_tenants.py`](06_isolate_two_tenants.py) | Many customers, one deployment: derived tenant ids, project-signed tokens, scoped keys, and per-project quota, attacked four ways. | core |
...
- **[`docs/books/`](../docs/books)** holds twelve executable notebooks, one
  per SPEC phase. They go considerably deeper and are run headlessly in CI.
- **[`docs/internal/SPEC.md`](../docs/internal/SPEC.md)** is the design
  contract, and
  [`docs/internal/DECISIONS.md`](../docs/internal/DECISIONS.md) records every
  pinned algorithm and id formula.
