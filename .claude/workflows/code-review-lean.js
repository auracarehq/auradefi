export const meta = {
  name: "code-review-lean",
  description: "Code review, token-tuned: finders pooled, semantically clustered BEFORE verification, verifiers handed the finder's reproduction to refute rather than rediscover. Same angles, same fan-out, same verdict ladder as the built-in code-review: roughly half the spend.",
...
const SCOPE_SCHEMA = {
  ...
  properties: {
    ...
    moduleMap: { type: "array", description: "One entry per changed file: its purpose and public surface, so finders need not each re-read the whole tree.", items: {
      type: "object", required: ["file", "purpose"],
      properties: {
        file: { type: "string" },
        purpose: { type: "string", description: "one line: what this module is for" },
        surface: { type: "string", description: "its exported/public symbols, comma-separated" },
      },
    }},
  },
}
  ...
  properties: {
    candidates: { type: "array", items: {
      ...
      properties: {
        ...
        repro: { type: "string", description: "If you actually reproduced this, the exact commands or script you ran and the output you observed, verbatim. Omit if you did not run it: never describe a reproduction you did not perform." },
      },
    }},
  },
}
...
const scope = await agent(
  ...
  "4. Build moduleMap: one entry per changed file with its purpose and public surface. Finders receive this instead of each independently reading every file, so it is worth doing properly: but do not read files in full where a symbol scan suffices.\n" +
...
const SCOPE_BLOCK =
  ...
  "## What changed\n" + scope.summary + "\n\n" +
  "## Module map (use this before opening a file: it is why you do not have to read the whole tree)\n" +
  ((scope.moduleMap || []).length > 0
    ? scope.moduleMap.map(m => "  - " + m.file + ": " + m.purpose + (m.surface ? " [" + m.surface + "]" : "")).join("\n") + "\n\n"
    : "  (none provided)\n\n") +
...
const FINDER_PROMPT = f => {
  ...
  return "## Code-review finder: " + f.label + "\n\n" + SCOPE_BLOCK + "\n" +
    ...
    "If you REPRODUCE a candidate, put the exact commands or script and the observed output in `repro`, verbatim: a verifier will re-run it instead of rebuilding your reasoning from scratch, which is the single cheapest thing you can do for this review. Never invent a repro you did not run. " +
...
const GROUP_VERIFIER_PROMPT = group =>
  ...
  group.map((c, i) =>
    ...
    (c.repro ? "\n    FINDER'S REPRODUCTION (re-run this first):\n" + c.repro : "") +
    ((c.clusterAlso || []).length > 0
      ? "\n    Same root cause also reported at: " + c.clusterAlso.join(", ") + ": your verdict applies to all of them."
      : "")
  ).join("\n") + "\n\n" +
  "Where a candidate carries a reproduction, RE-RUN IT FIRST and judge what you observe. Only investigate from scratch if it does not reproduce: in which case say so, because a repro that does not reproduce is itself strong evidence for REFUTED. " +
...
async function verifyGroups(candidates) {
  ...
  const out = await parallel(groups.map(g => async () => {
    ...
    // A group whose every candidate arrives reproduced needs adjudication,
    // not investigation. Anything unreproduced keeps the session tier.
    const allReproduced = g.every(c => c.repro)
    const opts = { label: "verify:" + short + "(" + g.length + ")", phase: "Verify", schema: GROUP_VERDICT_SCHEMA }
    if (allReproduced) opts.effort = "medium"
    const r = await agent(GROUP_VERIFIER_PROMPT(g), opts)
    if (!r) return []
    const byIdx = {}
...
// ─── Cluster (barrier) → Verify ───
// The built-in verifies every candidate and merges same-root-cause duplicates
// afterwards, at synthesis. Measured on a 4.24M-token run: 43 verified
// findings collapsed to ~30 distinct defects, so ~13 verifier agents (~640k
// tokens, 15% of the run) adjudicated something already being adjudicated
// elsewhere. This does the same merge BEFORE the money: verify one
// representative per root cause and propagate its verdict to the cluster.
// It is the same merge, not a weaker one: nothing is dropped, and every
// merged location still reaches the report.
const CLUSTER_SCHEMA = {
  type: "object", required: ["clusters"],
  properties: {
    clusters: { type: "array", items: {
      type: "object", required: ["representative", "members"],
      properties: {
        representative: { type: "number", description: "[i] index of the best-described candidate for this root cause" },
        members: { type: "array", items: { type: "number" }, description: "[i] indices of every candidate sharing this root cause, INCLUDING the representative" },
...
async function clusterCandidates(cands) {
  if (cands.length < 2) return cands
  const listing = cands.map((c, i) =>
    "[" + i + "] " + loc(c) + ": " + c.summary + "\n      consequence: " + c.failure_scenario
  ).join("\n")
  const r = await agent(
    "## Cluster code-review candidates by root cause\n\n" +
    "Below are " + cands.length + " candidate findings from independent reviewers working different angles. " +
    "They overlap: the same defect is often reported two or three times, at the same line or at different lines that share one underlying cause.\n\n" +
    listing + "\n\n" +
    "## Instructions\n" +
    "Group the candidates by ROOT CAUSE. Two candidates share a root cause only if ONE code change would fix both.\n" +
    "- Every index must appear in exactly one cluster. A candidate with no partner is a cluster of one.\n" +
    "- Pick as `representative` the member whose summary and failure scenario are most precise and most complete: a verifier will judge the whole cluster from it.\n" +
    "- **Do not over-merge.** Same file, same function, or same general topic is NOT the same root cause. Two distinct bugs in one function are two clusters. When genuinely unsure, keep them separate: a wrong split costs one extra verification, a wrong merge hides a real defect.\n\n" +
    "Structured output only.",
    { label: "cluster", phase: "Cluster", schema: CLUSTER_SCHEMA, effort: "medium" }
  )
  if (!r || !Array.isArray(r.clusters) || r.clusters.length === 0) {
    log("cluster: agent returned nothing usable: verifying every candidate (built-in behaviour)")
    return cands
  }
  // Assemble defensively: an index the clusterer dropped, duplicated, or
  // invented must not silently remove a candidate from the review.
  const claimed = new Set()
  const reps = []
  for (const cl of r.clusters) {
    const members = (Array.isArray(cl.members) ? cl.members : [])
      .filter(i => inBounds(i, cands.length) && !claimed.has(i))
    if (members.length === 0) continue
    members.forEach(i => claimed.add(i))
    const repIdx = inBounds(cl.representative, cands.length) && members.includes(cl.representative)
      ? cl.representative
      : members[0]
    const others = members.filter(i => i !== repIdx)
    reps.push({ ...cands[repIdx], clusterAlso: others.map(i => loc(cands[i])), clusterSize: members.length })
  }
  // Anything the clusterer never mentioned is its own cluster. Never dropped.
  for (let i = 0; i < cands.length; i++) {
    if (!claimed.has(i)) reps.push({ ...cands[i], clusterAlso: [], clusterSize: 1 })
  }
  log("cluster: " + cands.length + " candidates -> " + reps.length + " root causes")
  return reps
}

phase("Cluster")
let representatives = await clusterCandidates(allCandidates)

// Rank before verifying so the report cap never discards work already paid
// for. The built-in verified everything and then dropped ~15 findings at the
// cap. Correctness outranks cleanup; the margin keeps headroom for refutations.
const VERIFY_MARGIN = 8
let unverifiedUnderCap = 0
if (representatives.length > P.maxFindings + VERIFY_MARGIN) {
  const ordered = representatives.slice().sort((a, b) => (a.kind === "cleanup" ? 1 : 0) - (b.kind === "cleanup" ? 1 : 0))
  const keep = ordered.slice(0, P.maxFindings + VERIFY_MARGIN)
  unverifiedUnderCap = representatives.length - keep.length
  log("cap: verifying " + keep.length + " of " + representatives.length + " root causes; " + unverifiedUnderCap + " lowest-precedence left unverified and NOT reported")
  representatives = keep
}

let verified = await verifyGroups(representatives)
...
const stats = {
  ...
  rootCauses: representatives.length,
  unverifiedUnderCap,
}
...
for (const d of decisions) {
  ...
  const alsoLocs = merged.map(loc).concat(c.clusterAlso || [])
...
for (let i = 0; i < ranked.length && findings.length < P.maxFindings; i++) {
  ...
  const c = ranked[i]
  const bAlso = (c.clusterAlso || []).length > 0 ? " [same root cause also at: " + c.clusterAlso.join(", ") + "]" : ""
  findings.push({ file: c.file, line: c.line, summary: c.summary + bAlso, failure_scenario: c.failure_scenario, category: c.kind, verdict: c.verdict })
  backfilled++
}
