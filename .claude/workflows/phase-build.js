export const meta = {
  name: 'phase-build',
  description: 'Build one spec phase: interpret -> blind gate -> waves of (test -> implement -> review) -> mutation proof -> seam audit -> pattern sweep -> ship',
  ...
  properties: {
    ...
    findings: {
      ...
      items: {
        ...
        properties: {
          severity: { enum: ['blocker', 'major', 'minor'] },
          kind: { enum: ['vacuous-test', 'unimplemented-pin', 'unmutatable-pin'] },
          test: { type: 'string' },
          pin: { type: 'string' },
          mutant: { type: 'string' },
...
const brief = (order) => JSON.stringify(order, null, 2)
...
async function buildOrder(order) {
  ...
  let implReport = await agent(
    `${ROLE('implementer')}\n\n${CONTEXT}\n\nStubs and red tests are in place. Your work order:\n${spec}\n\nTest-author report:\n${testReport}`,
