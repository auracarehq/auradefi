...
**Yes — your own API, your own database, your own prices, your own clock.**
Every collaborator is a port, and a port is a plain object with one or two
methods. No base class to inherit, no registration, no decorator: the
protocols are structural, so satisfying the shape *is* implementing them.

The defaults exist so you do not have to start here. Replace one port and
keep the rest:
...
`ledger` is where transactions live. The shipped SQL backend takes a
**session factory, not a URL**, and that is deliberate: your application owns
the engine, the connection pool and the migrations. auradefi never opens a
connection you did not hand it and never emits DDL, which is also why there
is no `AURADEFI_DATABASE_URL` to set.
...
Three behaviours a replacement has to copy, because callers depend on them:

1. **`upsert` is idempotent.** Re-ingesting an unchanged transaction emits no
   event. A whole tick must be safe to retry.
2. **A removed row that returns is re-added**, not mutated: stored with
   `removed=False`, a bumped sequence, and an `ADDED` event. That is how a
   reorg stays expressible.
3. **`sync` pages by last-modified order, not transaction date**, so an old
   row that changes reappears at the end of the feed. Clients page until
   `has_more` is `False` before persisting the cursor.

`get` for another tenant's id must raise `NotFoundError` — indistinguishable
from a row that never existed, so an id cannot be used as an existence
...
`source` is one object with two methods. You may not have to write it:
...
**The engine owns the window.** It chooses the blocks, the page number and
the sort order, and it learns a window has drained by receiving a page
shorter than `offset`. So answer the window you were asked for: never widen
it, never page internally, never retry silently. Returning everything at
once defeats the budget; returning an empty page early advances a cursor over
data you never read.

Rows come back **raw** (`list[dict]`) because parsing belongs to the decoder
seam, which you can also replace via `decoder=`.

For failures, raise `auradefi.errors.SourceError` — or any `AuradefiError` —
and `sync()` contains it to that one connection's report row. Anything else
propagates, because a `KeyError` in your adapter is a bug and hiding it would
be worse than a loud tick.
...
`report.unpriced`, never valued at zero. This is the port to bind if you need
Bitcoin or Solana prices, which the default cannot provide at all.

Use `Money` with exact `Decimal` amounts. Never a float: the whole point of
the arithmetic in this package is that it does not drift.
...
implementation is deliberately not written yet, and this is the port to bind
if you want durable cursors before it is.
...
`tenants()` is the one method with no `tenant_id`, and it is load-bearing:
`sync()` enumerates work from the **store**, not from process memory. A
worker that read its tenant list from memory would restart, find nothing, and
report a cheerful `no_op` forever. That was a real defect (0.1.1 #21).
...
`FrozenClock(ms)` ships for tests and replays; `SystemClock` is the default.
Time being a port is why quota windows, sync throttling and `as_of_ms` are
all testable without sleeping, and why Sandbox can hand you reproducible
answers.
...
- **The chain registry is per-instance and mutable** (`register()` a chain and
  `connect_address` will accept it), but the seeded set is five chains and the
  decoder needs an entry to exist before a connection can be made.
- **The decoder** is replaceable via `decoder=`, but the shipped one handles
  EVM native txlist rows only.
- **Position adapters** need a `ContractReader` you supply — no `eth_call`
  transport and no multicall ship in this package, which is the largest gap
  between "works" and "works against mainnet".
